...
if not CREDS:
    ...
    for t in transactions:  # response.result.transaction_details:
        ...
        udate = (
            parse_date(t.transaction_info.transaction_updated_date)
            .astimezone(tz.tzlocal())
            .strftime("%Y-%m-%d")
        )
        ...
        if amount > 0:
            if hasattr(t, "payer_info"):
                ...
                if hasattr(t.payer_info, "address"):
                    addr = [
                        getattr(t.payer_info.address, "line1", "").replace(
                            "No Street Address Provided", ""
                        ),
                        getattr(t.payer_info.address, "city", "").replace(
                            "No City Provided", ""
                        ),
            ...
            for item in cart.item_details:
                ...
                if wait_for_amount:
                    if item_name == "Amount":
                        ...
                        raise Exception(
                            "Oops: Expecting an Amount item following a transaction without an amount and didn't find one."
                        )
                else:
                ...
                if hasattr(item, "item_amount"):
                    ...
                    raise Exception(
                        "Oops: Was waiting for an Amount item, it came, but didn't actually contain an amount!"
                    )
                else:
