...
class TransactionRequest:
    ...
    def __init__(self, dt_from, dt_to, max_days=MAX_DAYS):
        ...
        while dt_slice < dt_to:
            ...
            self.slices.append(
                f"start_date={quote(start.isoformat())}&end_date={quote((end-timedelta(seconds=1)).isoformat())}"
            )
            dt_slice = end

        self.path_root = (
            f"/v1/reporting/transactions?fields=all&page_size={MAX_PAGE_SIZE}&"
        )
    ...
    def execute(self, client=None, environment=None):
        if not client:
            if not environment:
                raise Exception(
                    "To execute a TransactionRequest a client or evironment must be provided."
                )
            else:
